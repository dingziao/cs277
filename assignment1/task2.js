...
// ################ Edit your code below
    var modelViewProjectionMatrix = new SimpleMatrix()
        .multiply(projection)
        .multiply(SimpleMatrix.inverse(view))
        .multiply(model);
    var viewportMatrix = SimpleMatrix.viewport(canvas.width, canvas.height);
    var fullTransformChain = new SimpleMatrix()
        .multiply(viewportMatrix)
        .multiply(modelViewProjectionMatrix);
// ################
...
// ################ Edit your code below
        var projectedxyz1 = fullTransformChain.multiplyVector(xyz1);
        var projectedxyz2 = fullTransformChain.multiplyVector(xyz2);
        var uv1 = [projectedxyz1[0]/projectedxyz1[3], projectedxyz1[1]/projectedxyz1[3]];
        var uv2 = [projectedxyz2[0]/projectedxyz2[3], projectedxyz2[1]/projectedxyz2[3]];
// ################
...
Task2.prototype.render = function(canvas, gl, w, h)
{
    ...
    var view = SimpleMatrix
    .rotate(this.cameraAngle, 1, 0, 0)
    .multiply(
