...
// ################ Edit your code below
	...
	for(var i=1; i<this.nodes.length-2; i++) {
		...
		for(var t=0; t<this.numSegments; t++) {
			drawLine(this.ctx, 
				1/6*((t*para_increment)**3*(-x0+3*x1-3*x2+x3) + (t*para_increment)**2*(3*x0-6*x1+3*x2) + (t*para_increment)*(-3*x0+3*x2) + x0+4*x1+x2),
				1/6*((t*para_increment)**3*(-y0+3*y1-3*y2+y3) + (t*para_increment)**2*(3*y0-6*y1+3*y2) + (t*para_increment)*(-3*y0+3*y2) + y0+4*y1+y2),
				1/6*(((t+1)*para_increment)**3*(-x0+3*x1-3*x2+x3) + ((t+1)*para_increment)**2*(3*x0-6*x1+3*x2) + ((t+1)*para_increment)*(-3*x0+3*x2) + x0+4*x1+x2),
				1/6*(((t+1)*para_increment)**3*(-y0+3*y1-3*y2+y3) + ((t+1)*para_increment)**2*(3*y0-6*y1+3*y2) + ((t+1)*para_increment)*(-3*y0+3*y2) + y0+4*y1+y2),		
			);
		}
	}
// ################
